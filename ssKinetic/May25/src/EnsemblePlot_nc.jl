...
figure("Carbon_nc",figsize=(23,7))
figure("Energy_nc",figsize=(20,12.5))
figure("Amino_nc",figsize=(15,12.5))
...
Tsim = readdlm("Ensemble/Best/Tsim")
...
for j in 1:size(species,1)
    ...
    m = readdlm("Ens_nc/"*key*"_m")
    s = readdlm("Ens_nc/"*key*"_s")
    ...
    fill_between(vec(Tsim),vec(m+z*s),vec(max(0,m-z*s)),color="#D3D3D3")#lightgreen")
