...
# Vectorize, remove zeros, and take log
rates = vec(Rates)
rates = log(10,rates[find(rates)])
#rates = rates[find(rates)]
...
println("Median rate constant value is ",round(median(Rates[find(Rates)]),2)," mM/h")
println("Median catalytic rate is ",round(median(Rates[find(Rates)])/.00017/3600,2)," /s")
