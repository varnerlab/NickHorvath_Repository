...
variance = (rem(j,3) == 1)?.05:((rem(j,3) == 2)?.1:.15)
...
#    all_indices = [1 22 25 26 42 58 59 68 69 70 99 110 111 112 113 118 120 132 134 135 137 138 139] # List of reactions to vary
    all_indices = [1 25 26 42 59 68 69 70 99 111 112 138 139] # List of reactions to vary
#    all_indices = collect(1:163)
...
#    data_dictionary["CONTROL_PARAMETER_ARRAY"] = reshape(cont,2,convert(Int64,length(cont)/2))'
    ...
    if time_elapsed < 11 # Should prevent sets with CVODE errors from joining ensemble
